...
2. `rng-seed` owns RNG algorithm/version, named stream derivation and serialized stream state. Request stable streams from that owner using semantic subsystem and global chunk identities; do not derive a second random algorithm here or let request order choose outcomes. This generator stores its versioned recipe plus edit deltas and the RNG stream references/state required for continuation. A seed alone does not guarantee reproducibility across algorithms, versions or physics backends.
